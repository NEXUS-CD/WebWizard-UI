...
// params 扁平化
const flatteningParams = (paramsCode: string) => {
  ...
  try {
    return JSON.parse(
      (paramsCode = paramsCode
        ...
        .replace(
          /\:([A-Za-z0-9_<>\.\|\&\']*?)\;/g,
          (current, matchTheChild) => {
            return matchTheChild ? `:"${matchTheChild}",` : matchTheChild;
          },
        )
        .replace(/([A-Za-z0-9_]*?)\:/g, (current, matchTheChild) => {
          return matchTheChild ? `"${matchTheChild}":` : matchTheChild;
        })
        .replace(/,(})/g, (current, matchTheChild) => {
...
export class FileStructures extends OriginFileStructures {
  ...
  getOriginFileStructures(
    ...
    dataSource.mods.forEach((mod) => {
      ...
      console.log(mod.name, modName, 248);
